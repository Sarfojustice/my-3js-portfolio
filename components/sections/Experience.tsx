...
export default function Experience() {
  ...
  useGSAP(() => {
    ...
    itemsRef.current.forEach((item, index) => {
      ...
      // Persistent 3D Mouse Interaction
  ...
  return (
    <section ref={sectionRef} id="experience" className="py-32 px-6 max-w-6xl mx-auto relative z-20">
      ...
      <div className="space-y-32 relative" style={{ perspective: "1500px" }}>
        {CV_DATA.experience.map((exp, index) => (
          <div 
            key={index}
            ref={(el) => { itemsRef.current[index] = el; }}
            className="group relative pl-12 md:pl-20 py-4"
          >
            ...
            <div className="experience-content p-10 bg-white/[0.02] border border-white/5 backdrop-blur-[4px] relative group-hover:bg-white/[0.04] group-hover:border-white/20 transition-all duration-500" style={{ transformStyle: "preserve-3d", backfaceVisibility: "hidden" }}>
              {/* Corner Accents */}
              ...
              <div className="flex flex-col md:flex-row md:items-baseline justify-between mb-4" style={{ transform: "translateZ(30px)" }}>
                <h3 className="text-3xl font-bold text-white tracking-tight group-hover:text-cyan-400 transition-colors">{exp.role}</h3>
                <span className="text-[10px] font-mono text-cyan-500/40 tracking-[0.3em] uppercase">{exp.period}</span>
              </div>
              
              <div className="flex items-center justify-between mb-10" style={{ transform: "translateZ(20px)" }}>
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-8" style={{ transform: "translateZ(10px)" }}>
